...
def check_region_type(region_name):
    if ((region_name.count(':') == 1) and ('-' not in region_name)):
        ...
        return 'junctions'
def calculate_exon_min_read_mapped_length(exon_region_name,point_dict,READ_JUNC_MIN_MAP_LEN,exon_position):
    assert '-' not in exon_region_name
    points = exon_region_name.split(':')
    if len(points) == 2:
        return READ_JUNC_MIN_MAP_LEN
    if exon_position == 'left':
        return point_dict[points[-1]] - point_dict[points[1]] + 1 + READ_JUNC_MIN_MAP_LEN
    elif exon_position == 'right':
        return point_dict[points[-2]] - point_dict[points[0]] + 1 + READ_JUNC_MIN_MAP_LEN
    elif exon_position == 'center':
        return point_dict[points[-2]] - point_dict[points[1]] + 1 + 2*READ_JUNC_MIN_MAP_LEN
            ...
            if check_region_type(region_name) in ['one_exon','two_exons']:
                ...
                if (first_exon_length > READ_JUNC_MIN_MAP_LEN) and (last_exon_length > READ_JUNC_MIN_MAP_LEN):
                    if region_name.count(':') == 2:
                        ...
                        if calculate_exon_min_read_mapped_length(exon_1,point_dict,READ_JUNC_MIN_MAP_LEN,exon_position='left') + calculate_exon_min_read_mapped_length(exon_2,point_dict,READ_JUNC_MIN_MAP_LEN,exon_position='right') <= max_read_len:
                            is_region_valid = True
            elif check_region_type(region_name) == 'exons':
                if calculate_exon_min_read_mapped_length(region_name,point_dict,READ_JUNC_MIN_MAP_LEN,exon_position='center') <= max_read_len:
                    is_region_valid = True
                ...
                if (first_exon_length > READ_JUNC_MIN_MAP_LEN) and (last_exon_length > READ_JUNC_MIN_MAP_LEN):
                    if exon_length + calculate_exon_min_read_mapped_length(exons[0],point_dict,READ_JUNC_MIN_MAP_LEN,exon_position='left') + calculate_exon_min_read_mapped_length(exons[-1],point_dict,READ_JUNC_MIN_MAP_LEN,exon_position='right') <= max_read_len:
                        is_region_valid = True
        else:
            is_region_valid = True
