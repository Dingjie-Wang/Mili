...
def TransELS(ref_file_path,short_read_alignment_file_path,long_read_alignment_file_path,output_path,region_expression_calculation_method,alpha,beta,P,filtering,threads=1,READ_LEN=150,READ_JUNC_MIN_MAP_LEN=10):
    ...
    with open('{}/dict.pkl'.format(output_path),'wb') as f:
        pickle.dump([SR_gene_regions_dict,short_read_gene_regions_read_count],f)
    SR_read_len = READ_LEN
    end_time_2 = time.time()
    print('Mapped {} short reads'.format(num_SRs),flush=True)
    print('Done in %.3f s'%(end_time_2-end_time_1),flush=True)
    ...
    print('Done in %.3f s'%(end_time_3-end_time_2),flush=True)
    ...
    print('Done in %.3f s'%(end_time_4-end_time_3),flush=True)
